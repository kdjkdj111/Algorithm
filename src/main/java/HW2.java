...
class Tree23<K extends Comparable<K>, V> {
	...
	private Node<K, V> search23(K key) {
		if (root == null) {
			return null;
		}
	...
	private int compare(K x, Node<K, V> t) {
	...
	public void put(K key, V value) {
		...
		if (root == null) root = new Node<K, V>(key, value);
			...
			while (true) {
				if (p.key_r == null) {
					if (newNode.key_l.compareTo(p.key_l) > 0) {
	...
	private void split(Node<K, V> p, Node<K, V> newNode) {
		K minK, midK, maxK;
		V minV, midV, maxV;
	...
	private void resetSize(Node<K, V> x) {
		...
		int count = 0;
		if (x.key_l != null) count++;
		if (x.key_r != null) count++;
	...
	private void rotate(Node<K, V> p, Node<K, V> q, Node<K, V> r) {
	...
	private void combine(Node<K, V> p, Node<K, V> q, Node<K, V> r) {
		if (p == r.right_child) {
			q.key_r = r.key_r;
			q.val_r = r.val_r;
